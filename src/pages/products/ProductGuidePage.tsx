...
const DATA: Record<ProductGuideId, {
  ...
  special: {
    ...
    cards: [
      { title: '방폭형 EX', tag: 'Ex e mb Ⅱ T1', body: '화학공장, 도장공장, 위험물 저장 시설 등 방폭 검토가 필요한 현장에 적용하는 제품군입니다.' },
...
export default function ProductGuidePage({ pageId }: ProductGuidePageProps) {
  ...
  return (
    <main style={{ background: '#F8FAFC', minHeight: '100vh' }}>
            <SubHero
        breadcrumb={[{ label: '제품안내' }, { label: page.title }]}
        badge={page.eyebrow}
        title={page.title}
        lead={page.desc}
        keywords={[page.title, '설치 방식 비교', '공간별 추천', '용량 산출 기준']}
      />
