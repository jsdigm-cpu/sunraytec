...
export type AdminTab = 'products' | 'content' | 'inquiries' | 'cases' | 'resources' | 'members' | 'hero-theme' | 'notices' | 'faqs' | 'posts';
...
export default function Sidebar({ tab, onTab, newInquiryCount = 0 }: Props) {
  ...
  return (
    <aside style={{ width: 260, background: '#374151', color: '#fff', padding: '1rem', flexShrink: 0 }}>
      ...
      <button style={itemStyle(tab === 'resources')} onClick={() => onTab('resources')}>
        <span>📄 자료실 관리</span>
      </button>
      ...
      <button style={itemStyle(tab === 'faqs')} onClick={() => onTab('faqs')}>
        <span>❓ FAQ관리</span>
      </button>
      <button style={itemStyle(tab === 'posts')} onClick={() => onTab('posts')}>
        <span>📰 뉴스·블로그</span>
      </button>
